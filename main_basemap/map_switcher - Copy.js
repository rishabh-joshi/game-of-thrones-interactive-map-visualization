(function (root, factory) {
    ...
    L.TileLayer.Version = L.TileLayer.extend({
        initialize: function (arg, options) {
            var versions = L.TileLayer.Version.versions;
            ...
            // overwrite values in version from variant.
            if (variantName && 'variants' in versions[versionName]) {
                if (!(variantName in versions[versionName].variants)) {
                    throw 'No such variant of ' + versionName + ' (' + variantName + ')';
                }
                var variant = versions[versionName].variants[variantName];
                var variantOptions;
                ...
                version = {
                    url: variant.url || version.url,
                    options: L.Util.extend({}, version.options, variantOptions)
                };
            }

            // If retina option is set
            if (version.options.retina) {
                // Check retina screen
                if (options.detectRetina && L.Browser.retina) {
                    ...
                    version.options.retina = '';
                }
            }

            // replace attribution placeholders with their values from toplevel version attribution,
            // recursively
            var attributionReplacer = function (attr) {
                ...
                return attr.replace(/\{attribution.(\w*)\}/,
                    function (match, attributionName) {
                        return attributionReplacer(versions[attributionName].options.attribution);
                    }
                );
            };
            version.options.attribution = attributionReplacer(version.options.attribution);
    ...
    L.tileLayer.change = function (version, options) {
        return new L.TileLayer.Version(version, options);
    };
